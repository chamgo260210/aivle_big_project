...
add('1. 구현 요약','기존 프로젝트형 재무분석 CRUD는 유지하면서, 프론트에서 독립 검증 가능한 무상태 모듈 API를 추가했다. http://localhost:3000/module 에서 금액 단위와 핵심 가정을 입력하면 API가 KRW로 변환하고 기준·보수·낙관 시나리오, 월별 현금흐름, 민감도 및 몬테카를로 위험분포를 반환한다.')
add('2. 모듈 구조와 책임'); table(['경로','역할'], [('backend/.../financial/module/FinancialModuleController.java','POST /api/v1/modules/financial/preview. UI 샌드박스 진입점.'),('.../FinancialModuleService.java','스케일링, 계산, 차트 및 보고서 DTO 조립.'),('.../FinancialInputScaler.java','입력 KRW/천원/백만원을 KRW로 단일 변환.'),('.../FinancialMonteCarloService.java','정규 충격을 적용한 반복 시뮬레이션과 분위수 계산.'),('.../module/dto/*.java','요청/응답 계약. 화면과 계산 구현의 결합 방지.'),('frontend/.../FinancialModulePage.jsx','/module 입력·결과 확인 화면.'),('backend/application.yaml','개발 origin CORS 허용.')])
