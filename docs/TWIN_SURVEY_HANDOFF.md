...
## 13. 「시장 인터뷰」로 교체 — 2026-08-12

**사용자 결정: 우열 비교(A vs B)는 여정 7번 자리에서 내린다.** 그 자리에 필요한 것은
「어느 쪽이 이기나」가 아니라 **확정된 사업안을 던져 사람들의 반응을 듣는 것**이었다.
업계 용어로 컨셉 테스트이고, 이 저장소에서는 `MARKET_INTERVIEW` 다.

§1~§12 는 **이력이다.** 우열 조사 코드는 지우지 않았고 도달 불가로 남아 있다.

### 13-1. 왜 정량이 아니라 정성인가 — 근거 둘

컨셉 테스트는 두 갈래다.

| | 정량 컨셉 테스트 | **정성 심층 인터뷰(IDI)** ← 채택 |
|---|---|---|
| 얻는 것 | 구매의향 top-2-box, 매력도·차별성·신뢰성 (5점) | 첫인상·이해도·끌림·걸림·장벽·개선안 |
| 해석 | **규범 DB(norms) 없으면 불가능** — 62%가 좋은지 나쁜지 모른다 | 언어 그대로가 결과물 |
| n | 100~300 | 8~20 |

정량을 못 쓰는 이유가 두 겹이다.

1. **이 저장소의 자체 실측**(§9) — 우열 3쌍은 세 계기가 전부 일치했고 **절대 임계를 묻는
   가격형 2쌍은 셋 다 갈렸다**. 구매의향 5점은 정확히 「절대 임계를 모델에게 묻는」 질문이라
   그 반증에 직격당한다.
2. **합성 응답자 문헌(2025~26)** — 총량 평균은 그럴듯하나 **분산이 죽고**, 사회적으로
   바람직한 답으로 쏠리고, 보기 순서 같은 설계 인공물에 흔들린다. 적합 용도는 초기
   스크리닝·저위험이라는 것이 문헌의 일치된 결론이다.

→ **이 조사가 내는 유일한 수치는 언급 수**다. 백분율로 환산하지 않는다. 계약·뷰모델·
골든 테스트 세 겹이 비율 칸이 생기는 것을 막는다.

### 13-2. 구조

> **⚠ 이 절은 §14(2026-08-13, 9문항·3층 개편)로 대체됐다.** 아래는 초판 구조다.

```
확정 사업안 스냅샷 ──(LLM 0회, 결정론)──▶ 컨셉보드 6칸  ← 화면에서 손볼 수 있다
                                              │
   1단계 응답 수집  n명 × 1셀 ────────────────┤ 카드 + 컨셉보드 + 고정 7문항 → 7칸 JSON
   2단계 주제 코딩  LLM 1회  ◀────────────────┘
                    ├ 이해도 3분류 + 오해 지점
                    ├ 주제 4축(끌림·걸림·장벽·개선) + 언급 수 + 대표 인용
                    └ 현재의 대안
```

**자극 초안(`TWIN_STIMULUS_DRAFT`)이 필요 없어졌다.** 쌍을 지어낼 일이 없으므로 컨셉보드는
스냅샷에서 결정론적으로 렌더한다 — LLM 호출 하나가 통째로 사라졌다.

**LLM 은 이름표만 붙인다.** 언급 수는 응답자 id 목록의 길이로 코드가 세고, 인용문은 그
응답자가 실제로 쓴 문장에서 코드가 꺼낸다. 보낸 id 와 대조해 없는 id 는 버린다.

### 13-3. 질문 가이드 (고정 7문항 · 바이트 동결)

> **⚠ 9문항으로 늘어났다. 현재 가이드는 §14-2 다.** 아래는 초판이다.

1. **첫인상** — 「방금 보신 것에 대해 떠오르는 대로」 *(무편집. 뒤 문항이 오염시키기 전)*
2. **이해** — 「이 제품이 뭘 해주는 건지 본인 말로」 *(반응이 나쁜 것과 설명이 나쁜 것을 가른다)*
3. **끌리는 점** · 4. **걸리는 점** · 5. **필요성**(현재 대안을 끌어냄) ·
   6. **안 사는 이유** · 7. **바꾸고 싶은 것**

초판 프롬프트 해시는 `8ec56f3a…35d6` 이었다. 현재 값은 §14-2.

### 13-4. 만든 것

| 층 | 경로 |
|---|---|
| AI | `ai/app/interview/` — `models`·`questions`·`runner`·`coding`·`caveats`·`__init__` |
| AI 테스트 | `ai/tests/test_interview_{questions,coding,survey,golden}.py` (61개) |
| 골든 | `ai/tests/fixtures/market_interview/interview.json` — **세 층이 함께 읽는다** |
| 백엔드 | `journey/MarketInterview{Run,Version,*Repository,Service,InputFactory,BoardService,Worker,Controller}.java` |
| 계약 | `taskrun/contract/MarketInterviewContract.java` + `MarketInterviewContractTests.java` (14개) |
| DB | `V23__market_interview.sql` — `market_interview_{runs,versions}` |
| 프론트 | `frontEnd/src/features/market-interview/` (8파일 + 테스트 2, 21개) |
| 스모크 | `scripts/market-interview-smoke.ps1` |

### 13-5. 우열 조사와 의도적으로 다른 곳

| | 우열 조사 | 시장 인터뷰 |
|---|---|---|
| 표본 | 50/100/300 | **20/40/80** (1인 1셀) |
| 셀 수 | n×쌍×2방향×2.2 (n=50·2쌍 → 477) | **n** (n=20 → 20 + 코딩 1) |
| 출력 형식 | 평문 「선택: A」 (계기 동결) | **구조화 출력**(json_schema) |
| 토큰 파라미터 | `max_tokens` | **`max_completion_tokens`** (§9 지뢰를 여기서만 고쳤다) |
| 게이트 | `SERVICEABLE={DOMINANCE}` — 유형별 차단 | 차단 없음. **윤리·가치 소구는 경고만** |
| 통계 | Δ·λ·MDE·위치편향 | 없음 — 언급 수뿐 |

윤리·가치형을 **차단이 아니라 경고**로 바꾼 이유: 우열 조사에서는 「그 축을 빼고 다시
물어라」가 가능했지만, 여기서는 자극이 확정된 사업안 그 자체다. 친환경이 본질인 사업안을
조사 자체에서 막는 것은 과하다. 대신 그 축의 반응을 믿지 말라고 값 옆에 적는다.

### 13-6. 남은 것 / 알려진 한계

- **유료 왕복 실측을 안 했다.** 프롬프트가 규칙을 얼마나 지키는지(코딩이 id 를 지어내지
  않는지, 이해도 판정이 사람다운지)는 아직 안 쟀다. `scripts/market-interview-smoke.ps1 -Paid`
  가 그것을 재는 자리다. 무료 구간은 전부 통과 확인했다
- **계기 동등성은 여전히 미확인**이다(`INSTRUMENT_EQUIVALENCE_CONFIRMED=False` 승계).
  거기에 더해 **이 조사 형식은 외적 타당성 시험을 한 번도 거치지 않았다** — 우열형의
  4/4 성적조차 여기로 전이되지 않는다. 경계 문구가 그렇게 적고 있다
- **마케팅(8번)으로 결과가 나가지 않는다.** 범위 밖(기존 결정 승계)
- ⚠ **`TwinSurveyContract` 에 잠복 결함 하나를 발견했다.** `sampling.shortCells` 의 값은
  정수가 아니라 `{quota, available}` 객체인데(`ai/app/twin/bank.py:117`) 계약이 그 자리를
  정수로 검사한다. 층이 얕아 목표를 못 채우는 실행을 계약이 거절한다 — 젊은 층이 마르는 큰
  표본에서만 드러난다. **고치지 않았다**(우열 조사는 도달 불가라 범위 밖). 되살릴 때 볼 것

---

## 14. 9문항 + Fact/Insight/So-What 3층 — 2026-08-13

**§13 이 아니라 이 절이 현재다.** §13 은 초판 이력이다.

사용자 진단이 둘이었다. ①「너무 나열식이라 실제 정보는 없다」 ② 실행 결과에서
**모든 주제가 `40명 중 40명`** 으로 나왔다.

### 14-1. 40/40 — 원인과 조치

n=40 실행에서 주제가 전부 40/40 이었고, 대안 3개(「가끔 요리한다」·「간편식·배달」·
「그냥 참는다」)가 **동시에** 40/40 이었다. 한 사람이 셋을 다 한다는 뜻이라 응답 분포로는
성립하지 않는다. 같은 판에서 **이해도만 22/17/1 로 멀쩡히 갈렸다** — 그것이 결정적 단서다.

| # | 원인 | 자리 |
|---|---|---|
| **A** | 이해도에만 배타 제약이 있었다(「셋 중 하나에 정확히 한 번」). **주제에는 아무 제약도 없었다** | 옛 `coding.py` SYSTEM_PROMPT |
| **B** | 규칙 3「숫자를 쓰지 말고 id 만 넣어라」를 모델이 **「전부 넣어라」로 수행**했다. 환각을 막으려던 지시가 덤프를 불렀다 | 같은 곳 |
| **C** | 검산이 「그 id 가 존재하나」만 봤다. **「그 사람이 그 말을 했나」는 아무도 안 봤다** | 옛 `_valid_ids` |

**조치는 프롬프트 문구가 아니라 구조다 — 「주제 → 사람」을 「사람 → 주제」로 뒤집었다.**

1. **1패스 코드북** — 전원 응답을 보고 **이름표 목록만** 만든다.
   스키마에 `respondentIds` 칸이 **아예 없다.** 덤프할 자리가 존재하지 않는다.
2. **2패스 배정** — 코드북을 고정한 채 응답자를 **한 명 한 줄로** 판정한다.
   `alternativeLabel` 이 **단수**라 「1인 1대안(합계 ≤ n)」이 구조로 강제된다.
   배정은 40명씩 쪼개도 안전하다(코드북이 고정이라 이름표가 안 갈린다).
3. **포화 게이트**(`saturation.py`, LLM 0회) — 그래도 진짜로 전원이 같은 말을 하는 경우는
   남는다. 그때는 막지 않고 **화면 맨 위에 「이 축은 읽지 마라」 경고**를 띄운다.
4. **계약 2건 추가** — `mentionCount == respondentIds.size()`,
   `alternatives` 언급 수 **합계 ≤ answered`. 옛 계약은 3×40/40 을 통과시켰다.

LLM 호출: **n + 1 + ⌈n/40⌉ + 1** (응답 n · 코드북 1 · 배정 ⌈n/40⌉ · 타겟 조건식 1).

### 14-2. 질문 가이드 (고정 9문항 · 바이트 동결)

| # | key | 무엇 |
|---|---|---|
| 1 | `firstImpression` | 첫인상 *(무편집)* |
| 2 | `restatement` | 본인 말로 설명 *(반응이 나쁜 것과 설명이 나쁜 것을 가른다)* |
| 3 | `like` | 끌리는 점 **+ 왜 본인에게 중요한지** |
| 4 | `concern` | 걸리는 점 **+ 왜, 무엇과 비교해서** |
| 5 | `differentiation` | **차별성**(신규) — 「다른 게 없다면 없다고 하셔도 된다」 |
| 6 | `relevance` | 필요성 + 현재의 대안 |
| 7 | `usageScene` | **사용 장면**(신규) — 「**상상해서**」 |
| 8 | `barrier` | 안 사는 이유 **+ 그게 없어지면 살 것인지** |
| 9 | `suggestion` | 바꾸고 싶은 것 |

**파고들기를 문항에 내장한 이유**: 1인 1턴이라 조사원이 되물을 기회가 없는데, 되묻기가
정성 조사의 본체다. 이유 사슬이 원문에 남아야 화면이 「가격이 비싸다 19명」에서 멈추지 않는다.

프롬프트 sha256: `192e6f518153dedf0530b48c04ecd28003ee968af492a96a85f91d4418266e25`
(`ai/tests/test_interview_questions.py` 가 못박는다). ⚠ 같이 올린 상수 하나:
`runner.MAX_COMPLETION_TOKENS` **1200 → 2000** — 안 올리면 답이 잘려 사람이 분모에서
빠지고, 절반이 빠지면 조사 전체가 `MARKET_INTERVIEW_NO_USABLE_RESPONSE` 로 죽는다.

### 14-3. 결과 봉투 — 최상위 10칸 → 16칸

```
conceptBoard · sampleSize · sampling · targeting · comprehension · differentiation ·
themes · alternatives · segments · contrast · suggestionLinks ·
interviews · transcripts · telemetry · caveats · notes
```

**`themes[].respondentIds` 가 이 개편의 심장이다.** 옛 구조는 언급 수를 센 뒤 명단을 버렸고,
그래서 「가격 우려 19명이 누구인가」를 물을 수 없었다. 명단만 남기면 나머지는 덧셈과
교집합이라 **AI 를 한 번도 더 부르지 않는다**(`analysis.py`).

| 층 | 블록 | 무엇 |
|---|---|---|
| — | `targeting` | 타겟/비타겟 8:2. **조건식을 화면에 그대로 보인다** — 기계가 옮긴 것이라 틀릴 수 있다 |
| Fact | `themes`(6축)·`differentiation`·`alternatives` | 집계 그대로. 대안은 1인 1개 |
| **Insight** | `segments`·`contrast` | **LLM 0회.** 주제 × 프로필 4축 교차, 타겟/비타겟 대비 |
| **So-What** | `suggestionLinks` | 제안 ↔ 우려·장벽. 근거는 **같은 사람이 둘 다 말했다**뿐 |
| 근거 | `interviews`(≤5)·`transcripts`(전원) | 전원 응답이 검증 통로다 |

지킨 것: **비율 칸은 여전히 하나도 없다.** `overlapCount` 라는 이름을 쓴 이유가 그것이다 —
`sharedCount` 는 세 층의 `share` 금지 검사에 걸린다(사람 수인데도).

봉투 크기 **실측**: n=20 에 56KB. n=80 추정 ~180KB. `ai/main.py` 의 2 MiB 상한은
**응답에도 걸리므로**(`RESPONSE_BYTES_EXCEEDED`) 확인이 필요했고, 여유가 크다.

### 14-4. 응답 원장 — 다음번엔 유료로 다시 재지 않는다

이번 40/40 을 원인까지 짚는 데 **유료 1판**이 들었다. 원장이 있었다면 0원이었다.

- `ai/app/interview/ledger.py` — `MARKET_INTERVIEW_LEDGER_DIR` 이 비면 **아무것도 안 한다**
  (운영 기본 꺼짐). 쓰기 실패는 삼킨다. **pid·카드 원문을 안 쓴다**(KMP 재배포 금지)
- `python -m app.tools.recode_ledger <원장>` — 응답을 그대로 두고 **코딩만** 다시 돌린다.
  n=40 이면 42회가 **2회**로 떨어진다. `--stored` 는 호출 0회, `--diff` 는 두 판 비교
- 판정은 진단표 한 장: 축별 이름표 수 · 축별 최대 언급 수 · 대안 합계 · 포화 목록.
  파이프라인과 **같은** `saturation.homogeneity` 를 쓴다 — 잣대가 갈리면 비교가 무의미하다

### 14-5. 만든 것 / 고친 것

| 층 | 무엇 |
|---|---|
| AI 신규 | `interview/{targeting,analysis,saturation,ledger}.py` · `tools/recode_ledger.py` |
| AI 개편 | `interview/{questions,models,coding,runner,caveats,__init__}.py` |
| AI 테스트 | `test_interview_{questions,coding,survey,golden,analysis,saturation,targeting,ledger}.py` — **120개** |
| 골든 | `_regenerate.py` 로 다시 굽는다(**유료 0회·결정론**). 손으로 고치지 말 것 |
| 계약 | `MarketInterviewContract.java` 16칸 + `MarketInterviewContractTests.java` **22개** |
| 프론트 | `marketInterviewResult.js`(정규화기) · `MarketInterviewPage.jsx`(3층) · CSS · 카드 9칸 |
| 스모크 | `market-interview-smoke.ps1` — `questions=9` + **40/40 재발 검사 4건** |
| DB | **없다.** `result_json` 이 TEXT 통째 저장이라 V24 가 필요 없었다 |

곁다리로 고친 실재 버그: `market-interview.css` 가 **정의되지 않은 토큰 3개**를 폴백 없이
쓰고 있었다(`--radius-sm`·`--radius-md`·`--color-bg-surface`). 선언 자체가 무효라 모서리와
배경이 조용히 빠져 있었다. `.mi-page` 지역 변수로 매핑했다.

### 14-6. 남은 것

- **유료 왕복 실측을 아직 안 했다.** 무료 구간(파이썬 120 · 백엔드 22 · 프론트 27)은 전부
  통과. `scripts/market-interview-smoke.ps1 -Paid` 를 **원장을 켜고** 돌릴 것.
  거기서 눈으로 볼 것은 §14-1 의 재발 여부이고, 스모크가 그 넷을 자동으로 잡는다
- **n=80 은 아직 안 돌렸다.** 코딩 프롬프트가 8칸 × 300자 × 80명으로 커지고
  (`FIELD_MAX_CHARS` 400→300, `CODING_BUDGET_SECONDS` 180→300 으로 대응),
  배정이 2배치가 된다. 한 번에 통과하는지는 유료 전에는 모른다
- **표집 재현성이 약해졌다.** 타겟 조건식을 LLM 이 만들므로 같은 사업안을 두 번 조사해도
  표집틀이 갈릴 수 있다. 조건식을 봉투에 박아 두어 **왜 갈렸는지는 보이게** 했고,
  재현성 테스트는 「조건식 고정 상태」로 범위를 명시했다
- `sampleSize.js` 의 소요 시간 어림은 **7문항 시절 값**이다. 유료 1판의 실측치로 다시 잡을 것
- 마케팅(8번)으로 결과가 나가지 않는 것은 그대로다(범위 밖)
