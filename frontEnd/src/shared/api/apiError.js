...
const USER_MESSAGE_BY_CODE = {
  ...
  // ⚠ **이 코드는 「실패」가 아니라 「이미 했다」다.** 같은 멱등키로 다른 내용을 보내면 난다 —
  //   대개 성공한 요청을 사용자가 다시 누른 경우다. 기본 문구(「잠시 후 다시 시도해 주세요」)로
  //   떨어지면 **영영 되지 않을 재시도를 시키고**, 성공한 일을 실패로 읽게 만든다
  //   (2026-08-16 실측: 컨셉 확정이 이미 성공해 있었는데 화면은 실패만 반복해 보였다).
  IDEMPOTENCY_CONFLICT: '이미 처리된 요청이에요. 화면을 새로고침해 현재 상태를 확인해 주세요.',
};
