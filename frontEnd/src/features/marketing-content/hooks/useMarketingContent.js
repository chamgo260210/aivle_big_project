...
export default function useMarketingContent(projectId) {
  ...
  const uploadReference = async (file) => {setState((value) => ({...value, uploading: true, error: null, }));
    try {
      const artifact = await api.uploadReference(
        projectId, file, { timeoutMs: 30000 }
      );
      setState((value) => ({...value, uploading: false, }));
      return artifact;
    } catch (error) {
      setState((value) => ({...value, uploading: false, error, }));
      throw error;
    }
};
