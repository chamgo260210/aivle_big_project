...
export default function MarketingSetupPanel({ value, onChange, onSubmit, disabled, busy }) {
  ...
  return <section className="mk-panel mk-setup" aria-labelledby="mk-setup-title">
    ...
    <Field label="참고 상품 이미지 (선택)" hint="PNG 또는 JPG, 최대 20MB · 제품 외형과 패키지를 살리고 싶을 때 첨부하세요."><input type="file" accept="image/png,image/jpeg" onChange={setImage}/>{value.referenceImage && (<small>{value.referenceImage.name}</small>)}</Field>
