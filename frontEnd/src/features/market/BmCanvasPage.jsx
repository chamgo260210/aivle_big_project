...
// ⚠ BM 실행계획 폼과 그 모델은 `concept-portfolio/` 가 정본이다 — 사업안 단계에서도
//    같은 칸을 쓰기 때문이다. 여기로 복사하지 말 것(둘이 갈리면 조용히 다른 값이 간다).
import BmPlanForm from '../concept-portfolio/BmPlanForm.jsx';
...
import { draftFrom, emptyCellNames, emptyDraft, toPayload } from '../concept-portfolio/bmPlan.js';
import './market.css';
...
export default function BmCanvasPage() {
  ...
  // 다듬기는 캔버스가 **있을 때만** 읽는다 — 없으면 읽을 것도 없고, 결말이 난 화면을
  // 계속 두드리면 아무것도 안 바뀌는 조회로 서버만 때린다.
  const revision = useConceptRevision(client, api, projectId, Boolean(result));
  ...
  // ★ **계획은 사업안 확정 자리에서 받는다**(`BusinessProposalWorkspace`). 버튼 하나가
  //   계획 저장과 가설 확정을 그 순서로 하므로, 여기 오기 전에 이미 채워져 있다.
  //   그래서 결과가 없다고 계획 화면을 **강제로** 띄우지 않는다 — 그렇게 하면 이 화면이
  //   막다른 길이 된다(`BmPlanForm` 에 제출 버튼이 없어 빠져나갈 수 없었다).
  //
  // ⚠ 그래도 **고칠 길은 남긴다.** 없애 버리면 한 번 돌린 프로젝트는 계획을 고칠 수 없고,
  //    빈 칸이 영영 빈 채로 남는다(실측: 사용자가 그 상태를 봤다). 「실행 계획 고치기」로만
  //    들어오고, 들어오면 저장하고 다시 생성할 수 있다.
  if (editingPlan) {
    return (
      <PlanPhase projectId={projectId} navigate={navigate} plan={plan}
        error={error} run={run} onBack={() => setEditingPlan(false)} />
    );
  }
    ...
    <ProjectWorkspace as="section" mode="analyze" className="market-page">
      ...
      {!result ? null : (
        <>
          ...
          {/* 컨셉 다듬기 — **탭을 새로 내지 않고 이 탭의 구획으로** 선다.
              캔버스를 본 «그 자리»에서 「이렇게 고치자」를 읽는 것이 순서에 맞고,
              칸을 새로 내면 백엔드가 모르는 모듈이 생겨 여정 2번이 영영 완료로 안 바뀐다. */}
          {revision.loading
            ? <LoadingState label="다듬어진 컨셉을 불러오는 중" />
            : (
              <RefinementSummary
                ...
                /* 근거는 시장조사 탭에 있다 — 같은 앵커 접두사(`sec-`)로 건너간다. */
                onJumpSubject={(anchor) =>
                  navigate(`${projectRoutes.market(projectId)}#${anchor}`)}
                onBack={null}
    ...
    <section className="market-page">
      ...
      <div className="bm-plan__split">
        <Card title="추가로 필요한 것">
          <BmPlanForm draft={plan.draft} onChange={plan.change} busy={plan.saving} />
          {/* ⚠ 제출 버튼은 **여기** 있다. `BmPlanForm` 은 갖지 않는다 — 그 폼은 사업안
              확정 자리에도 살고, 거기서는 「7개 검증 가정 확인」 버튼 하나가 저장과 확정을
              같이 한다. 폼이 자기 버튼을 가지면 그쪽에 버튼이 둘이 된다. */}
          <div className="mr-actions">
            <Button onClick={plan.submit} disabled={plan.saving}>
              {plan.saving ? '저장 중…' : '저장하고 다시 생성'}
            </Button>
          </div>
        </Card>
