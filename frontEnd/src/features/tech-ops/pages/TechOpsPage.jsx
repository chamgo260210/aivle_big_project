...
function CommercializationAdvisory({ techOps, safe }) {
  ...
  return <section className="tech-ops-advisory" aria-labelledby="tech-ops-advisory-title">
    <div className="tech-ops-section__heading"><div><p>출시 준비</p><h2 id="tech-ops-advisory-title">기술·운영 자문</h2></div><span>{advisory?.stale ? '업데이트 필요' : ({ QUEUED: '대기 중', READY: '대기 중', RUNNING: '분석 중', SUCCEEDED: '완료', COMPLETED: '완료', NEEDS_INPUT: '입력 필요', FAILED: '확인 필요', CANCELLED: '취소됨', TIMED_OUT: '시간 초과' })[advisory?.status] ?? '시작 전'}</span></div>
