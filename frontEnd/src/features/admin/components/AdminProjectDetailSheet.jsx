...
import { adminModuleLabel } from '../model/adminLabels.js';
...
export default function AdminProjectDetailSheet({ projectId, onRequestClose }) {
  ...
  return <SideSheet open phase={phase} onExited={finishClose} onClose={close}
    ...
    <nav className="admin-breadcrumb" aria-label="현재 위치"><Link to="/admin">관리자</Link> / <Link to="/admin/projects">프로젝트 관리</Link> / 상세</nav>
    <p id={descriptionId} className="admin-sheet-description">공통 프로젝트 정보와 6단계 프로젝트 모듈 상태를 확인합니다.</p>
    ...
    {!loading && project && <div className="admin-project-detail">
      ...
      <section><h3>소유자</h3><dl className="admin-detail-list">
        <dt>이름</dt><dd>{project.owner.displayName || '—'}</dd><dt>사용자 아이디</dt><dd>{project.owner.username ? `@${project.owner.username}` : '—'}</dd>
      </dl></section>
      ...
      <section><h3>프로젝트 단계</h3><div className="admin-analysis-list">
        {project.modules.map((module) => <div className="admin-analysis-row" key={module.module}>
          <strong>{adminModuleLabel(module.module)}</strong><AdminStatusBadge value={module.status} />
          {module.activeRunId && <small>실행 ID: {module.activeRunId}</small>}
          {module.sourceSnapshotId && <small>기준 스냅샷 ID: {module.sourceSnapshotId}</small>}
          {module.updatedAt && <small>최근 변경: {date(module.updatedAt)}</small>}
        </div>)}
      </div></section>
      <section><h3>운영 식별자</h3><dl className="admin-detail-list"><dt>프로젝트 ID</dt><dd>{project.id}</dd></dl></section>
    </div>}
  </SideSheet>;
}
