...
def test_발행사를_모르면_한_회사라고_단정하지_않는다(promote):
    """**틀린 경계는 없는 것보다 나쁘다.**

    실측(2026-08-15): 산업 전체 합계인 「식품제조업 매출액 188.8조」에
    「⚠ 시장 전체가 아니라 **한 회사**의 수다」가 붙었다. 사업가는 그 수의 정체를
    **반대로** 읽는다. 그 실행에서 `COMPETITOR_FIRM` 승격 7건이 전부 발행사 미상이었다.
    """
    d = _publish()
    d["문서별"][0]["items"][0]["게재_발행사"] = None

    카드 = promote.build(d)
    경계 = " ".join(카드[0]["경계"])
    # 「한 회사의 수인지 산업 합계인지 가리지 못했다」는 **단정이 아니라 유보**다.
    # 막으려는 것은 단정문 쪽이다.
    assert "한 회사**의 수다" not in 경계 and "한 회사의 수다" not in 경계, \
        "발행사를 모르는데 한 회사라고 단정했다"
    assert "발행사를 확인하지 못했다" in 경계

    # 아는 경우에는 종전대로 이름을 박는다 — 약하게 만드는 것이 목적이 아니다.
    d2 = _publish()
    카드2 = promote.build(d2)
    assert "**오뚜기 한 회사**" in " ".join(카드2[0]["경계"])
