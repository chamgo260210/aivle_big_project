...
public class MarketingStrategyService {
    ...
    public StrategyActionResponse start(
        ...
        if (inputJson.getBytes(StandardCharsets.UTF_8).length
                > MAX_INPUT_BYTES) {
            // 어느 소스가 큰지 말해 주지 않으면 상한만 보고는 고칠 수가 없다.
            StringBuilder breakdown = new StringBuilder();
            JsonNode measured = source.sources();
            measured.propertyNames().forEach(name -> breakdown
                .append(name).append('=')
                .append(mapper.writeValueAsString(measured.path(name))
                    .getBytes(StandardCharsets.UTF_8).length / 1024)
                .append("kB "));
            log.warn("Marketing strategy source too large projectId={} total={}kB {}",
                projectId, inputJson.getBytes(StandardCharsets.UTF_8).length / 1024, breakdown);
            throw new BusinessException(
