...
public class MarketingContentService {
    ...
    /** 저장 시 생성되는 키 형식. 다른 키를 열지 않도록 여기서 한 번 더 확인한다. */
    private static final String GENERATED_IMAGE_KEY =
     ...
     * 생성된 키비주얼을 화면에 돌려준다.
     *
     * <p>여태 이 길이 없었다. 이미지는 만들어져 저장까지 됐는데(`ai-artifacts/{uuid}.jpg`)
     * 꺼내는 엔드포인트가 없어, 화면은 저장소 키를 그대로 {@code <img src>} 에 넣었고
     * 브라우저는 그것을 상대경로로 풀어 SPA 폴백 HTML 을 받아 깨진 이미지만 보여 줬다.
     *
     * <p>⚠ 저장소 키를 <b>파라미터로 받지 않는다.</b> 소유가 확인된 콘텐츠의 최신 개정에서
     * 직접 꺼낸다 — 키를 받으면 남의 프로젝트 이미지를 요청할 수 있게 된다.
     */
    @Transactional(readOnly = true)
    public GeneratedImage image(Long ownerId, Long projectId, String id) {
        requireOwned(ownerId, projectId);
        MarketingContent content = find(projectId, id);
        MarketingContentRevision latest = revisions
            .findFirstByContentIdAndDeletedAtIsNullOrderByRevisionNumberDesc(content.getId())
            .orElseThrow(() -> new BusinessException(ErrorCode.RESOURCE_NOT_FOUND, "생성된 이미지가 없습니다."));
        JsonNode refs = mapper.readTree(latest.getResultJson()).path("artifactRefs");
        String objectKey = refs.isArray() && !refs.isEmpty() ? refs.get(refs.size() - 1).asText("") : "";
        if (!objectKey.matches(GENERATED_IMAGE_KEY))
            throw new BusinessException(ErrorCode.RESOURCE_NOT_FOUND, "생성된 이미지가 없습니다.");
        try {
            long size = objectStorage.metadata(objectKey).sizeBytes();
            return new GeneratedImage(objectStorage.open(objectKey), size);
        } catch (java.io.IOException | RuntimeException failure) {
            throw new BusinessException(ErrorCode.RESOURCE_NOT_FOUND, "생성된 이미지를 읽을 수 없습니다.");
        }
    }

    public record GeneratedImage(java.io.InputStream stream, long sizeBytes) {}
