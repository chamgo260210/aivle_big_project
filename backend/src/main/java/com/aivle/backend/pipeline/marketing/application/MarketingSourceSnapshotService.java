...
import com.aivle.backend.pipeline.marketing.api.MarketingSourceApiModels.SnapshotView;
...
public class MarketingSourceSnapshotService {
    ...
    public SnapshotView finalizeSnapshot(Long ownerId, Long projectId) {
        ...
        var existing =
            sources.findBySourceMarketSeedSnapshotIdAndProjectIdAndDeletedAtIsNull(
                marketSeed.getId(),
                projectId
            );
        ...
        MarketingSourceSnapshot source;

        if ("CONCEPT_PORTFOLIO_V2".equals(marketSeed.getSourceType())) {
            var concept =
                portfolioConcepts.findByIdAndProjectIdAndDeletedAtIsNull(
                    marketSeed.getPortfolioConceptId(),
                    projectId
                ).orElseThrow(
                    () -> new BusinessException(
                        ErrorCode.CONCEPT_NOT_SELECTABLE
                    )
                );

            var built = factory.create(id, now, marketSeed, concept);

            source = MarketingSourceSnapshot.createPortfolio(
                id,
                projectId,
                marketSeed.getId(),
                marketSeed.getPortfolioSelectionId(),
                marketSeed.getPortfolioConceptId(),
                MarketingSourceSnapshotFactory.SCHEMA_VERSION,
                built.hash(),
                mapper.writeValueAsString(built.body()),
                ownerId,
                now
            );
        } else {
            var concept =
                concepts.findByIdAndProjectIdAndPublishedTrueAndDeletedAtIsNull(
                    marketSeed.getConceptId(),
                    projectId
                ).orElseThrow(
                    () -> new BusinessException(
                        ErrorCode.CONCEPT_NOT_SELECTABLE
            ...
            source = MarketingSourceSnapshot.create(
                id,
                projectId,
                marketSeed.getId(),
                marketSeed.getSelectionId(),
                marketSeed.getConceptId(),
                MarketingSourceSnapshotFactory.SCHEMA_VERSION,
                built.hash(),
                mapper.writeValueAsString(built.body()),
    ...
    private com.aivle.backend.pipeline.marketseed.domain.MarketAnalysisSeedSnapshot
            currentMarketSeed(Long projectId) {

        var portfolioSelection =
            portfolioSelections
                .findByProjectIdAndIsCurrentTrueAndDeletedAtIsNull(projectId);

        if (portfolioSelection.isPresent()) {
            return marketSeeds
                .findByPortfolioSelectionIdAndStaleAtIsNullAndDeletedAtIsNull(
                    portfolioSelection.get().getId()
                )
                .filter(value ->
                ...
                .filter(value ->
                    portfolioSelection.get().getConceptId()
                        .equals(value.getPortfolioConceptId())
                )
                .orElseThrow(
                    () -> new BusinessException(
                    ErrorCode.HYPOTHESIS_DECISIONS_INCOMPLETE
                    )
                );
        }
        ...
        var selection =
            selections
                .findByProjectIdAndCurrentSelectionTrueAndDeletedAtIsNull(
                    projectId
                )
                .orElseThrow(
                    () -> new BusinessException(
                        ErrorCode.CONCEPT_SELECTION_REQUIRED
                    )
                );

        return marketSeeds
            .findBySelectionIdAndProjectIdAndDeletedAtIsNull(
                selection.getId(),
                projectId
            )
            .orElseThrow(
    ...
    private SnapshotView view(MarketingSourceSnapshot value) {
        ...
        return new SnapshotView(
            MarketingSourceSnapshotFactory.CONTRACT,
            value.getId(),
            value.getSchemaVersion(),
            value.getProjectId(),
            portfolio
                ? value.getPortfolioSelectionId()
                : value.getSelectionId(),
            portfolio
                ? value.getPortfolioConceptId()
                : value.getConceptId(),
            value.getSourceMarketSeedSnapshotId(),
            value.getSnapshotHash(),
            value.getFinalizedAt(),
