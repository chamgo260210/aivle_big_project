...
public class TechOpsAdvisorySourceResolver {
    ...
    public Sources resolve(Long ownerId, Long projectId) {
        ...
        MarketResearchVersion market = marketRun == null ? null : marketVersions.findBySourceRunIdAndDeletedAtIsNull(marketRun.getId())
            ...
            .orElse(null);
        // 시드가 있는데 시장조사가 안 잡히면 계보가 끊긴 것이다. 여기서 막지 않으면 아래 Worker 가
        // marketResult 를 빈 객체로 넣어 「시장 자료 없이 성공한 자문」이 유료로 만들어진다.
        if (seed != null && market == null) throw unavailable(
            "현재 사업안과 이어지는 시장조사 결과가 없습니다. 시장조사를 최신 사업안으로 다시 실행한 뒤 진행해 주세요.");
