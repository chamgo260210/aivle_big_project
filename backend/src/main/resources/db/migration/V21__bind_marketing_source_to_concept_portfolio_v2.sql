    ...
    ADD CONSTRAINT ck_marketing_source_authority
        CHECK (
            (
                ...
                AND portfolio_selection_id IS NULL
                AND portfolio_concept_id IS NULL
            )
            OR
            (
                source_type = 'CONCEPT_PORTFOLIO_V2'
                AND selection_id IS NULL
                AND concept_id IS NULL
                AND portfolio_selection_id IS NOT NULL
                AND portfolio_concept_id IS NOT NULL
            )
        );